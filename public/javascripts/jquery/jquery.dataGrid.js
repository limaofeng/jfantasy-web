jQuery.fn.extend({
    ...
    dataGrid: (function () {
        ...
        var _templateHtml = '<div class="grid_wrapper">' +
            ...
            '<table class="header formTable mb3 listTable" style="margin-left: 0px;"><thead></thead></table>' +
            '<table class="optheader formTable mb3 listTable" style="margin-left: 0px;display: none;"><thead></thead></table>' +
            '</div>' +
            '</div>' +
            ' <div class="grid_scrollBody scrollable-content">' +
            ' <table class="grid_view formTable mb3 listTable" style="margin-left: 0px; width: 100%;">' +
            '      <thead>' +
            ...
            '   </div>' +
                /*
            '<div class="grid_scrollFoot ui-state-default" style="overflow: hidden; border: 0px none; width: 100%;display: none;">' +
            '    <div class="grid_scrollFootInner">' +
            '       <table class="formTable mb3 listTable" style="margin-left: 0px;">' +
            '           <tfoot>' +
            '               <tr>' +
            '               </tr>' +
            '           </tfoot>' +
            ...
            */
            ...
            '<div id="pager" class="paging digg pages"></div>' +
            '</div>' +
            '</div>';

        return function ($search, $batch,settings) {
            settings = Fantasy.copy({height:function(){
                $('.grid_scrollBody',_$dataGrid)
                return 'calc(100% - '+$('.grid_scrollHead',_$dataGrid).height()+'px)';//+ $('.grid_scrollFoot',_$dataGrid).height()
            }},settings);
            ...
            if($('caption',$(this)).length){
                $('table.optheader,table.header',$('.grid_scrollHeadInner',_$dataGrid)).append($('caption',$(this)));
            }

            $('.grid_scroll',_$dataGrid).css({'height':'calc(100% - '+($('.grid_search',_$dataGrid).height() + $('.grid_pager',_$dataGrid).height())+'px)'})
