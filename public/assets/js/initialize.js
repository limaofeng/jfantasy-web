...
//				allowFileManager : true,
                ...
                items: ['source' , 'preview', '|', 'bold', 'italic', 'underline', 'strikethrough', 'fontname', 'fontsize', 'forecolor', 'hilitecolor', 'plug-align', 'plug-order', 'plug-indent', '|', 'image','baidumap']
            });
    ...
    jQuery.fn.extend({
        ...
        list : function($savForm,json){
            ...
            return {
                ...
                getData : function(){
                    return listView.getData().filter(function(){
                        delete this.listIndex;//去掉额外添加的字段
                        return true;
                    });
                }
            };
    ...
    Fantasy.Ajax.on('error', function (xhr, status, err, errorHeader, s) {
        if ((status == 200 || status == 403) && err == 'struts2 action Error') {
            var errorMsg = '';
